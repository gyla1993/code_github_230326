...
def GFR(X, K, C, alpha, L, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
...
def GF(X, K, C, L, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
    X = X - np.mean(X, axis=0)
    E = calc_dist(X, X)
    S_hat = modified_softmax_normalized(E, K)
    A_hat = np.matmul(S_hat, S_hat.T)
    if save_path is not None:
        with open(save_path, 'w') as f:
            f.write('')
    for l in range(L):
...
def PIC(X, K, C, L, save_path=None, label=None):
    best_nmi = -1
    best_nmi_iter = -1
    X = X - np.mean(X, axis=0)
    E = calc_dist(X, X)
    A_hat = row_normalized_affinity(E, K)
    if save_path is not None:
        with open(save_path, 'w') as f:
            f.write('')
    v = np.matmul(A_hat, np.ones((X.shape[0], 1), dtype=float))
...
if __name__ == "__main__":
    ...
    K_list = [4, 8, 16, 32]
    K_list = [16, 32]
    dn_list = ['Iris', 'Seed', 'Wine', 'OrlFace10', 'OrlFace40']
    dn_list = ['OrlFace10', 'OrlFace40']
    for dn in dn_list:
        ...
        if method_opt == 'GFR':
            ...
            for K in K_list:
                for alpha in alpha_list:
                    ...
                    GFR(data, K, C, alpha, L, save_path=rec_dn_dir + para_str + '.txt', label=label)
        if method_opt == 'GF':
            L = 30
            for K in K_list:
                para_str = 'K({:02})'.format(K)
                GF(data, K, C, L, save_path=rec_dn_dir + para_str + '.txt', label=label)

        if method_opt == 'PIC':
            L = 120
            for K in K_list:
                para_str = 'K({:02})'.format(K)
                PIC(data, K, C, L, save_path=rec_dn_dir + para_str + '.txt', label=label)
